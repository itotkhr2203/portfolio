  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(
      ...
      (state, action: PayloadAction<PROFILE[]>) => {
        return {
          ...state,
          profiles: action.payload.map((prof) => {
            const res = prof.img ? prof.img!.replace('http:', 'https:') : null;
            return ({ id: prof.id, user_profile: prof.user_profile, img: res })
          }),
        };
      }
    );
        ...
        return {
          ...
          profiles: state.profiles.map((prof) => {
            ...
            if (prof.id === action.payload.id) {
              const res = action.payload.img!.replace('http:', 'https:');
              return ({ id: action.payload.id, user_profile: action.payload.user_profile, img: res })
            } else {
              return prof;
            }
          }
